...
//                 const sentimentData = JSON.parse(data.sentiment);  // Assuming `data.sentiment` contains the JSON string of `P`
//                 const sentimentHtml = sentimentData.map(item => {
//                     return `<p>Sentiment Type: ${item.index} - Percentage: ${item.percent}%</p>`;
//                 }).join("");
                ...
                document.getElementById('uploadButton').addEventListener('click', async () => {
                    try {
                        ...
                        const sentimentHtml = data.sentiment.map(item => {
                            return `<p>Sentence: ${item.Text}<br>Sentiment: ${item.Sentiment}<br>Speaker: ${item.Speaker}<br>Duration: ${item.Duration} seconds</p>`;
                        }).join("");
